...
from src.miscc.utils import build_super_images, mkdir_p
from src.datasets import TextDataset, prepare_data
from src.model import CNN_ENCODER, RNN_ENCODER
from zsl.models import zsl_d

dir_path = (os.path.abspath(os.path.join(os.path.realpath(__file__), './.')))
sys.path.append(dir_path)

LOG = setup_logger('./pretrain.log')
...
def train(dataloader, cnn_model, rnn_model, d_model, batch_size,
          labels, optimizer, d_optimizer, epoch, ixtoword, image_dir):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        # ZSL G Losses
        g_loss_adv_sent = F.binary_cross_entropy_with_logits(pred_by_sent, torch.zeros_like(pred_by_sent))
        g_loss_adv_emb = F.binary_cross_entropy_with_logits(pred_by_emb, torch.zeros_like(pred_by_emb))
        g_loss_cls_sent = F.cross_entropy(pred_class_by_sent, targets)
        g_loss_cls_emb = F.cross_entropy(pred_class_by_emb, targets)
        g_loss = (g_loss_adv_sent + g_loss_adv_emb + g_loss_cls_sent + g_loss_cls_emb)
        ...
        # ZSL D Losses
        d_loss_adv_sent = F.binary_cross_entropy_with_logits(pred_by_sent, torch.zeros_like(pred_by_sent))
        d_loss_adv_emb = F.binary_cross_entropy_with_logits(pred_by_emb, torch.ones_like(pred_by_emb))
        d_loss_cls_sent = F.cross_entropy(pred_class_by_sent, targets)
        d_loss_cls_emb = F.cross_entropy(pred_class_by_emb, targets)

        d_loss = (d_loss_adv_sent + d_loss_adv_emb + d_loss_cls_sent + d_loss_cls_emb)
        d_loss.backward()
        d_optimizer.step()
        ...
        if step % UPDATE_INTERVAL == 0:
            ...
            info = ('| epoch {:3d} | {:5d}/{:5d} batches | ms/batch {:5.2f} | '
                    's_loss {:5.2f} {:5.2f} | '
                    'w_loss {:5.2f} {:5.2f} | g_loss {:5.2f} d_loss {:5.2f}'.format(epoch,
                                             step, len(dataloader), elapsed * 1000. / UPDATE_INTERVAL,
                                             s_cur_loss0, s_cur_loss1,
                                             w_cur_loss0, w_cur_loss1, g_total_loss, d_total_loss)
                    )
...
def build_models():
    ...
    discriminator = zsl_d(cfg.ZSL.NUM_CLASSES)
...
if __name__ == "__main__":
    ...
    try:
        ...
        g_optimizer = optim.Adam(para, lr=lr, betas=(0.5, 0.999))
        d_optimizer = optim.Adam(discriminator.parameters(), lr=lr, betas=(0.5, 0.999))
        for epoch in range(start_epoch, cfg.TRAIN.MAX_EPOCH):
            epoch_start_time = time.time()
            count = train(dataloader, image_encoder, text_encoder, discriminator,
                          batch_size, labels, g_optimizer, d_optimizer, epoch,
