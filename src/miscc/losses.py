...
def generator_loss(netsD, image_encoder, zsl_discriminator, fake_imgs, real_labels,
    ...
    for i in range(numDs):
        ...
        if i == (numDs - 1):
            ...
            # zsl
            pred, pred_class = zsl_discriminator(cnn_code)
            targets = torch.LongTensor([c_id - 1 for c_id in class_ids]).to(pred.device)

            d_loss_adv = F.binary_cross_entropy_with_logits(pred, torch.zeros_like(pred))
            d_loss_cls = F.cross_entropy(pred_class, targets)
