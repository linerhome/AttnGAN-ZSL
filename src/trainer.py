...
from src.miscc.losses import discriminator_loss, generator_loss, words_loss
from src.miscc.utils import (build_super_images, build_super_images2,
                             copy_G_params, load_params, mkdir_p, weights_init)
from src.model import CNN_ENCODER, G_DCGAN, G_NET, RNN_ENCODER
from zsl.models import ZSLD
...
class condGANTrainer(object):
    ...
    def build_models(self):
        # ###################encoders######################################## #
        if cfg.TRAIN.NET_E == '':
            LOGGER.error('no pretrained text-image encoders')
            return
        ...
        LOGGER.info(f'Load image encoder from: {img_encoder_path}')
        image_encoder.eval()
        ...
        for p in text_encoder.parameters():
            p.requires_grad = False
        LOGGER.info(f'Load text encoder from: {cfg.TRAIN.NET_E}')
        text_encoder.eval()

        zsl_discriminator_path = cfg.TRAIN.NET_E.replace('text_encoder', 'discriminator')
        zsl_discriminator = ZSLD(cfg.ZSL.NUM_CLASSES)
        state_dict = torch.load(zsl_discriminator_path, map_location=lambda storage, loc: storage)
        zsl_discriminator.load_state_dict(state_dict)
        for p in zsl_discriminator.parameters():
            p.requires_grad = False
        zsl_discriminator.eval()
        LOGGER.info(f'Load ZSL Discriminator from: {zsl_discriminator_path}')
        ...
        LOGGER.info(f'# of netsD: {len(netsD)}')
        #
        epoch = 0
        if cfg.TRAIN.NET_G != '':
            ...
            LOGGER.info(f'Load G from: {cfg.TRAIN.NET_G}')
        ...
        if cfg.CUDA:
            ...
            zsl_discriminator = zsl_discriminator.cuda()
            netG.cuda()
            for i in range(len(netsD)):
                netsD[i].cuda()

        return [text_encoder, image_encoder, netG, netsD, zsl_discriminator, epoch]
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            it = tqdm.tqdm(range(self.num_batches)) if tqdm is not None else range(self.num_batches)
            for step in it:
        ...
        if cfg.TRAIN.NET_G == '':
            ...
            for key in data_dic:
                ...
                for i in range(1):  # 16
                    ...
                    # (2) Generate fake images
                    ######################################################
                    fake_imgs, attention_maps, _, _ = netG(sent_emb, words_embs, mask)
                    # G attention
